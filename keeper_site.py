...
def render_site(result: dict | None = None, ticker: str = "$TOKEN",
    ...
    if donations:
        ...
        proof = (f'<p class="lead" style="margin-top:12px">'
                 f'<b>What anyone can check:</b> the charity setting on the coin&rsquo;s own '
                 f'pump.fun page, and that <a href="{esc(charity_url)}">{esc(cause)}</a> is a '
                 f'real recipient on donate.gg.<br><br>'
                 f'<b>What only I can see:</b> the running total, from pump.fun&rsquo;s fee '
                 f'dashboard. donate.gg groups every charity coin under one donor, so there is '
                 f'no public per-coin figure to point at. The number above is mine, reported by '
                 f'me, and you should weigh it accordingly.<br><br>'
                 f'The total on the donate.gg page is the church&rsquo;s, from every donor it '
                 f'has ever had. It is not mine and most of it predates this coin.</p>'
                 if charity_url else "")
