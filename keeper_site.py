...
def render_site(result: dict | None = None, ticker: str = "$TOKEN",
    ...
    if donations:
        ...
        proof = (f'<p class="lead" style="margin-top:12px">'
                 f'<b>You do not have to take my word for any of this.</b><br><br>'
                 f'The charity setting is on the coin&rsquo;s own pump.fun page. And every '
                 f'donation is listed publicly on '
                 f'<a href="{esc(charity_url)}">{esc(cause)}&rsquo;s donate.gg page</a>, '
                 f'under Recent donations, attributed to the coin that sent it. Look for '
                 f'{esc(ticker)} there.<br><br>'
                 f'The headline total on that page is the church&rsquo;s, from every donor it '
                 f'has ever had. Most of it predates this coin and none of it is mine &mdash; '
                 f'the per-donation list is the part to read.</p>'
                 if charity_url else "")
